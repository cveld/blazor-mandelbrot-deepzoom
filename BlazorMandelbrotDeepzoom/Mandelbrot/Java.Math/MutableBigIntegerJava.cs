...
* @since   1.3
*/
public class MutableBigIntegerJava {
    /**
     ...
     */
    static MutableBigIntegerJava ONE = new MutableBigIntegerJava(1);
     ...
     */
    static int KNUTH_POW2_THRESH_LEN = 6;
     ...
     */
    static int KNUTH_POW2_THRESH_ZEROS = 3;
     ...
     */
    MutableBigIntegerJava(BigInteger b) {
        intLen = b.mag.Length;
     ...
     */
    MutableBigIntegerJava(MutableBigIntegerJava val) {
        intLen = val.intLen;
    ...
    private long toLong() {
        if (intLen > 2)
        {
            throw new Exception("this MutableBigInteger exceeds the range of long");
        }
        if (intLen == 0)
            return 0;
     ...
     * greater than {@code b}.
     */
    int compare(MutableBigIntegerJava b) {
        int blen = b.intLen;
        if (intLen < blen)
            return -1;
        if (intLen > blen)
     ...
     */
    private int compareShifted(MutableBigIntegerJava b, int ints) {
        int blen = b.intLen;
     ...
     * from divide().
     */
    readonly int compareHalf(MutableBigIntegerJava b) {
        int blen = b.intLen;
        int len = intLen;
        if (len <= 0)
            return blen <= 0 ? 0 : -1;
     ...
     */
    private readonly int getLowestSetBit() {
        if (intLen == 0)
            return -1;
        int j, b;
     ...
     * platforms.
     */
    private readonly int getInt(int index) {
     ...
     */
    private readonly long getLong(int index) {
     ...
     */
    void copyValue(MutableBigIntegerJava src) {
        int len = src.intLen;
        if (value.Length < len)
            value = new int[len];
     ...
     */
    private readonly void primitiveRightShift(int n) {
        int[] val = value;
        int n2 = 32 - n;
     ...
     * Assumes that intLen > 0, n > 0 for speed
     */
    private readonly void primitiveLeftShift(int n) {
        int[] val = value;
        int n2 = 32 - n;
     ...
     */
    void add(MutableBigIntegerJava addend) {
        int x = intLen;
        int y = addend.intLen;
     ...
     */
    void addShifted(MutableBigIntegerJava addend, int n) {
        if (addend.isZero()) {
            return;
        }
     ...
     * and {@code addend}.
     */
    void addDisjoint(MutableBigIntegerJava addend, int n) {
        if (addend.isZero())
            return;
     ...
     */
    void addLower(MutableBigIntegerJava addend, int n) {
        MutableBigIntegerJava a = new MutableBigIntegerJava(addend);
     ...
     */
    int subtract(MutableBigIntegerJava b) {
        MutableBigIntegerJava a = this;
     ...
     */
    private int difference(MutableBigIntegerJava b) {
        MutableBigIntegerJava a = this;
        int sign = a.compare(b);
        if (sign == 0)
            return 0;
     ...
     */
    void multiply(MutableBigIntegerJava y, MutableBigIntegerJava z) {
        int xLen = intLen;
        int yLen = y.intLen;
     ...
     */
    void mul(int y, MutableBigIntegerJava z) {
        if (y == 1) {
            z.copyValue(this);
            return;
        }
     ...
     *
     */
    int divideOneWord(int divisor, MutableBigIntegerJava quotient) {
     ...
     *
     */
    MutableBigIntegerJava divide(MutableBigIntegerJava b, MutableBigIntegerJava quotient) {
        return divide(b,quotient,true);
    }

    MutableBigIntegerJava divide(MutableBigIntegerJava b, MutableBigIntegerJava quotient, bool needRemainder) {
     ...
     */
    MutableBigIntegerJava divideKnuth(MutableBigIntegerJava b, MutableBigIntegerJava quotient) {
     ...
     *
     */
    MutableBigIntegerJava divideKnuth(MutableBigIntegerJava b, MutableBigIntegerJava quotient, bool needRemainder) {
        if (b.intLen == 0)
        ...
        if (intLen == 0) {
            ...
            return needRemainder ? new MutableBigIntegerJava() : null;
        }
        ...
        if (cmp < 0) {
            quotient.intLen = quotient.offset = 0;
            return needRemainder ? new MutableBigIntegerJava(this) : null;
        }
        ...
        if (b.intLen == 1) {
            ...
            if(needRemainder) {
                if (r == 0)
                    return new MutableBigIntegerJava();
                return new MutableBigIntegerJava(r);
            } else {
                return null;
            }
        }
        ...
        if (intLen >= KNUTH_POW2_THRESH_LEN) {
            ...
            if (trailingZeroBits >= KNUTH_POW2_THRESH_ZEROS*32) {
                MutableBigIntegerJava a = new MutableBigIntegerJava(this);
                b = new MutableBigIntegerJava(b);
                a.rightShift(trailingZeroBits);
                b.rightShift(trailingZeroBits);
                MutableBigIntegerJava r = a.divideKnuth(b, quotient);
     ...
     */
    MutableBigIntegerJava divideAndRemainderBurnikelZiegler(MutableBigIntegerJava b, MutableBigIntegerJava quotient) {
        int r = intLen;
        int s = b.intLen;
        ...
        if (r < s) {
            ...
            MutableBigIntegerJava bShifted = new MutableBigIntegerJava(b);
            bShifted.safeLeftShift(sigma);   // step 4a: shift b so its Length is a multiple of n
            MutableBigIntegerJava aShifted = new MutableBigIntegerJava (this);
            ...
            MutableBigIntegerJava a1 = aShifted.getBlock(t-1, t, n);   // the most significant block of a

            // step 7: z[t-2] = [a[t-1], a[t-2]]
            MutableBigIntegerJava z = aShifted.getBlock(t-2, t, n);    // the second to most significant block
            z.addDisjoint(a1, n);   // z[t-2]

            // do schoolbook division on blocks, dividing 2-block numbers by 1-block numbers
            MutableBigIntegerJava qi = new MutableBigIntegerJava();
            MutableBigIntegerJava ri;
            for (int i=t-2; i > 0; i--) {
            ...
            // readonly iteration of step 8: do the loop one more time for i=0 but leave z unchanged
            ri = z.divide2n1n(bShifted, qi);
            quotient.add(qi);
     ...
     * @return {@code this%b}
     */
    private MutableBigIntegerJava divide2n1n(MutableBigIntegerJava b, MutableBigIntegerJava quotient) {
        int n = b.intLen;
        ...
        MutableBigIntegerJava aUpper = new MutableBigIntegerJava(this);
        aUpper.safeRightShift(32*(n/2));   // aUpper = [a1,a2,a3]
        keepLower(n/2);   // this = a4

        // step 3: q1=aUpper/b, r1=aUpper%b
        MutableBigIntegerJava q1 = new MutableBigIntegerJava();
        MutableBigIntegerJava r1 = aUpper.divide3n2n(b, q1);

        // step 4: quotient=[r1,this]/b, r2=[r1,this]%b
        addDisjoint(r1, n/2);   // this = [r1,this]
        MutableBigIntegerJava r2 = divide3n2n(b, quotient);
     ...
     * @return {@code this%b}
     */
    private MutableBigIntegerJava divide3n2n(MutableBigIntegerJava b, MutableBigIntegerJava quotient) {
        int n = b.intLen / 2;   // half the Length of b in ints

        // step 1: view this as [a1,a2,a3] where each ai is n ints or less; let a12=[a1,a2]
        MutableBigIntegerJava a12 = new MutableBigIntegerJava(this);
        a12.safeRightShift(32*n);

        // step 2: view b as [b1,b2] where each bi is n ints or less
        MutableBigIntegerJava b1 = new MutableBigIntegerJava(b);
        b1.safeRightShift(n * 32);
        BigInteger b2 = b.getLower(n);

        MutableBigIntegerJava r;
        MutableBigIntegerJava d;
        if (compareShifted(b, n) < 0) {
            // step 3a: if a1<b1, let quotient=a12/b1 and r=a12%b1
            r = a12.divide2n1n(b1, quotient);

            // step 4: d=quotient*b2
            d = new MutableBigIntegerJava(quotient.toBigInteger().multiply(b2));
        } else {
            ...
            d = new MutableBigIntegerJava(b2);
            d.leftShift(32 * n);
            d.subtract(new MutableBigIntegerJava(b2));
        }
     ...
     * @return
     */
    private MutableBigIntegerJava getBlock(int index, int numBlocks, int blockLength) {
        ...
        return new MutableBigIntegerJava(newVal);
    }
     ...
     */
    long divide(long v, MutableBigIntegerJava quotient) {
        if (v == 0)
            throw new ArithmeticException("BigInteger divide by zero");
     ...
     */
    private MutableBigIntegerJava divideMagnitude(MutableBigIntegerJava div,
                                              MutableBigIntegerJava quotient,
                                              bool needRemainder ) {
        // assert div.intLen > 1
        // D1 normalize the divisor
        int shift = Integer.numberOfLeadingZeros(div.value[div.offset]);
        // Copy divisor value to protect divisor
        readonly int dlen = div.intLen;
        int[] divisor;
        MutableBigIntegerJava rem; // Remainder starts as dividend with space for a leading zero
        if (shift > 0) {
            divisor = new int[dlen];
            copyAndShift(div.value,div.offset,dlen,divisor,0,shift);
            if (Integer.numberOfLeadingZeros(value[offset]) >= shift) {
                int[] remarr = new int[intLen + 1];
                rem = new MutableBigIntegerJava(remarr);
                rem.intLen = intLen;
                rem.offset = 1;
            ...
            rem = new MutableBigIntegerJava(new int[intLen + 1]);
        ...
        readonly int limit = nlen - dlen + 1;
     ...
     * the remainder object is returned.
     */
    private MutableBigIntegerJava divideLongMagnitude(long ldivisor, MutableBigIntegerJava quotient) {
        // Remainder starts as dividend with space for a leading zero
        MutableBigIntegerJava rem = new MutableBigIntegerJava(new int[intLen + 1]);
        System.arraycopy(value, offset, rem.value, 1, intLen);
        rem.intLen = intLen;
        rem.offset = 1;
     ...
     * @since 9
     */
    MutableBigIntegerJava sqrt() {
        // Special cases.
        if (this.isZero()) {
            return new MutableBigIntegerJava(0);
        ...
        if (bitLength() <= 63) {
            ...
            do {
                ...
                if (xk1 >= xk) {
                    return new MutableBigIntegerJava(new int[] {
            ...
            MutableBigIntegerJava xk = new MutableBigIntegerJava(this);
            xk.rightShift(shift);
            xk.normalize();
            ...
            xk = new MutableBigIntegerJava(bi.mag);

            // Shift the approximate square root back into the original range.
            xk.leftShift(shift / 2);

            // Refine the estimate.
            MutableBigIntegerJava xk1 = new MutableBigIntegerJava();
            do {
                // xk1 = (xk + n/xk)/2
     ...
     */
    MutableBigIntegerJava hybridGCD(MutableBigIntegerJava b) {
        // Use Euclid's algorithm until the numbers are approximately the
        // same Length, then use the binary GCD algorithm to find the GCD.
        MutableBigIntegerJava a = this;
        MutableBigIntegerJava q = new MutableBigIntegerJava();

        while (b.intLen != 0) {
            if (Math.abs(a.intLen - b.intLen) < 2)
                return a.binaryGCD(b);

            MutableBigIntegerJava r = a.divide(b, q);
            a = b;
            b = r;
        }
        return a;
     ...
     */
    private MutableBigIntegerJava binaryGCD(MutableBigIntegerJava v) {
        // Algorithm B from Knuth section 4.5.2
        MutableBigIntegerJava u = this;
        MutableBigIntegerJava r = new MutableBigIntegerJava();
        ...
        // step B2
        bool uOdd = (k == s1);
        MutableBigIntegerJava t = uOdd ? v: u;
        int tsign = uOdd ? -1 : 1;
     ...
     * the operation.
     */
    MutableBigIntegerJava mutableModInverse(MutableBigIntegerJava p) {
        ...
        MutableBigIntegerJava oddMod = new MutableBigIntegerJava(p);
        ...
        MutableBigIntegerJava oddPart = modInverse(oddMod);

        // Calculate 1/a mod evenMod
        MutableBigIntegerJava evenPart = modInverseMP2(powersOf2);

        // Combine the results using Chinese Remainder Theorem
        MutableBigIntegerJava y1 = modInverseBP2(oddMod, powersOf2);
        MutableBigIntegerJava y2 = oddMod.modInverseMP2(powersOf2);

        MutableBigIntegerJava temp1 = new MutableBigIntegerJava();
        MutableBigIntegerJava temp2 = new MutableBigIntegerJava();
        MutableBigIntegerJava result = new MutableBigIntegerJava();
     ...
     */
    MutableBigIntegerJava modInverseMP2(int k) {
        if (isEven())
        ...
        if (k < 33) {
            t = (k == 32 ? t : t & ((1 << k) - 1));
            return new MutableBigIntegerJava(t);
        }
        ...
        MutableBigIntegerJava result = new MutableBigIntegerJava(new int[2]);
     ...
     */
    static MutableBigIntegerJava modInverseBP2(MutableBigIntegerJava mod, int k) {
        // Copy the mod to protect original
        return fixup(new MutableBigIntegerJava(1), new MutableBigIntegerJava(mod), k);
    }
     ...
     */
    private MutableBigIntegerJava modInverse(MutableBigIntegerJava mod) {
        MutableBigIntegerJava p = new MutableBigIntegerJava(mod);
        MutableBigIntegerJava f = new MutableBigIntegerJava(this);
        MutableBigIntegerJava g = new MutableBigIntegerJava(p);
        SignedMutableBigInteger c = new SignedMutableBigInteger(1);
        SignedMutableBigInteger d = new SignedMutableBigInteger();
        MutableBigIntegerJava temp = null;
     ...
     */
    static MutableBigIntegerJava fixup(MutableBigIntegerJava c, MutableBigIntegerJava p,
                                                                      int k) {
        MutableBigIntegerJava temp = new MutableBigIntegerJava();
     ...
     */
    MutableBigIntegerJava euclidModInverse(int k) {
        MutableBigIntegerJava b = new MutableBigIntegerJava(1);
        b.leftShift(k);
        MutableBigIntegerJava mod = new MutableBigIntegerJava(b);

        MutableBigIntegerJava a = new MutableBigIntegerJava(this);
        MutableBigIntegerJava q = new MutableBigIntegerJava();
        MutableBigIntegerJava r = b.divide(a, q);

        MutableBigIntegerJava swapper = b;
        // swap b & r
        b = r;
        r = swapper;

        MutableBigIntegerJava t1 = new MutableBigIntegerJava(q);
        MutableBigIntegerJava t0 = new MutableBigIntegerJava(1);
        MutableBigIntegerJava temp = new MutableBigIntegerJava();
