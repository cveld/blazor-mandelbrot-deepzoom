...
class SFTPaletteVeryOld : IPalette
{	
	public uint GetColour(int i)
	{
		uint u = (uint)i;
		return  (uint)0xff000000 + (uint)u + ((u * 7 & 255)<<8) + ((u * 31 & 255)<<16);
	}
	public uint GetAverageColour(int i0, int i1, int i2, int i3)
	{
		return 0;
	}
	public uint GetAverageColour(int i0, int i1, int i2, int i3,int i4, int i5, int i6, int i7, int i8)
	{
		return 0;
	}
}
...
class PaletteBand
{
	...
	public void Apply(int[] aColour, int i)
	{
		if (mWidth==0)
			return;
		...
		if (i>0 && i<mWidth)
		{
			aColour[0] = (int)(aColour[0] * mRed_scale);
			aColour[1] = (int)(aColour[1] * mGrn_scale);
			aColour[2] = (int)(aColour[2] * mBlu_scale);		
	...
	public void ParseString(String aString)
	{
		...
		mFirst = int.Parse(numbers[0]);
		mWidth = int.Parse(numbers[1]);
		mPeriod = int.Parse(numbers[2]);
		mRed_inc = int.Parse(numbers[3]);
		mGrn_inc = int.Parse(numbers[4]);
		mBlu_inc = int.Parse(numbers[5]);
		mRed_scale_int = int.Parse(numbers[6]);
		mGrn_scale_int = int.Parse(numbers[7]);
		mBlu_scale_int = int.Parse(numbers[8]);
...
public class SFTPaletteOld : IPalette
{
...
*/		
	...
	public uint GetAverageColour(int i0, int i1, int i2, int i3,int i4, int i5, int i6, int i7, int i8)
	{
		uint c1,c2,c3,c4,c5,c6,c7,c8,c9,c;
	...
	public void ParseString(String aString)
	{
		...
		if (!lines[0].Equals("sft_palette"))
			return;
		
		String[] numbers = lines[1].Split(',');
		mDr_di1 = float.Parse(numbers[0]);	
		mDr_di2 = float.Parse(numbers[1]);	
		mDecay_r = float.Parse(numbers[2]);	

		numbers = lines[2].Split(',');
		mDg_di1 = float.Parse(numbers[0]);	
		mDg_di2 = float.Parse(numbers[1]);	
		mDecay_g = float.Parse(numbers[2]);	
		
		numbers = lines[3].Split(',');
		mDb_di1 = float.Parse(numbers[0]);	
		mDb_di2 = float.Parse(numbers[1]);	
		mDecay_b = float.Parse(numbers[2]);	

		numbers = lines[4].Split(',');
		mStart_red = int.Parse(numbers[0]);	
		mStart_grn = int.Parse(numbers[1]);	
		mStart_blu = int.Parse(numbers[2]);	
		...
		mEnd_colour  = (uint)((r<<16)+(g<<8) + b) + 0xff000000;
