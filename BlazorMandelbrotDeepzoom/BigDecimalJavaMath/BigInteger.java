 ...
 * -2<sup>{@code Integer.MaxValue}</sup> (exclusive) to
 * +2<sup>{@code Integer.MaxValue}</sup> (exclusive)
...
public class BigInteger extends Number implements Comparable<BigInteger> {
     ...
     * range.
     */
    private static final int MAX_MAG_LENGTH = Integer.MaxValue / Integer.SIZE + 1; // (1 << 26)
    ...
    */
    ...
    boolean primeToCertainty(int certainty, Random random) {
        int rounds = 0;
        int n = (Math.min(certainty, Integer.MaxValue-1)+1)/2;
    ...
    */
    ...
    public short shortValueExact() {
        if (mag.length <= 1 && bitLength() <= 31) {
            int value = intValue();
            if (value >= Short.MIN_VALUE && value <= Short.MaxValue)
                return shortValue();
        }
    ...
    public byte byteValueExact() {
        if (mag.length <= 1 && bitLength() <= 31) {
            int value = intValue();
            if (value >= Byte.MIN_VALUE && value <= Byte.MaxValue)
                return byteValue();
        }
